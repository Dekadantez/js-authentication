  ...
  convertData = (data) => {
    return {
      ...
      list: data.list.map((user) => ({
        ...user, //передаємо всі дані юзера
        role: USER_ROLE[user.role], //передаємо необхідну роль
      })),
    }
  }

  updateView = () => {
    this.element.innerHTML = '' //прибираємо все що було до оновлення сторінки
    ...
    switch (this.status) {
      ...
      case this.STATE.SUCCESS:
        this.data.list.forEach((item) => {
          //додаємо ще один рядок в кінець списку
