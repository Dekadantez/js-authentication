...
router.post('/recovery', function (req, res) {
  ...
  if (!email) {
    //перевіряємо чи є в нас такий емейл, якщо ні відправляємо помилку
  ...
  try {
    //перевіряємо чи існує користувач з таким емайлом
    const user = User.getByEmail(email)
    if (!user) {
      return res.status(400).json({
...
router.post('/signup-confirm', function (req, res) {
  ...
  if (!code || !token) {
    return res.status(400).json({
      message: 'Помилка. Обовязкові дані відсутні',
    })
  }
  try {
